 ...
 * @param pageWordList Le nombre total de mots dans la phrase
 ...
 */
export function tfIdf(word: string, pageWordList: string[], docsWithWord: number, totalDocs: number): number {
    // TF
    const tf =
        pageWordList.filter(w => w === word).length /
        pageWordList.length;
